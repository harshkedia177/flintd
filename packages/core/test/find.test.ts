  ...
  test("stop() gives up on an embedding that ignores it, and the pass that wakes later writes nothing", async () => {
    ...
    try {
      ...
      const model = fakeModel({
        ...
        embedding: true,
        // Long enough that the 250 ms a search waits for a pass is told apart from the pass itself by half a
        // second, and short enough that the pass is over before this Library closes.
        embedPauseMs: 1200,
      })
